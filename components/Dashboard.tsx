...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-6xl w-full mx-auto px-6 py-10 flex flex-col gap-8">
      <header className="border-b hairline pb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="font-serif-brand text-3xl text-[var(--navy)]">Policy Due Desk</h1>
        ...
        <button
          onClick={() => setModal({ mode: "add", policy: null })}
          className="shrink-0 px-4 py-2 text-sm rounded-sm bg-[var(--navy)] text-white hover:bg-[var(--navy-deep)]"
        >
          + Add policy
        </button>
      </header>
      ...
      {loadError && (
        <div className="border border-[var(--danger)] text-[var(--danger)] rounded-sm p-4 text-sm">
          {loadError}
        </div>
      )}
